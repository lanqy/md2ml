...
async function main() {
  ...
  for (let item of files) {
    ...
    let separators = ['//', '\\\\', '\\', '/'];
    if (isWin) {
      pathArr = item.split(new RegExp(separators.join('|')));
    }else {
      pathArr = item.split('/');
    }
  ...
  for (let page of pageFiles) {
    ...
    let separators = ['//', '\\\\', '\\', '/'];
    page.replace(/\\/g,'/').replace(/\/\//g,'/').replace(/\\\\/g,'/')
    if (isWin) {
      pathArr = page.split(new RegExp(separators.join('|')));
    }else {
      pathArr = page.split('/');
    }
  ...
  let jsons = _.sortBy(jsonArr, 'created', 'desc').reverse(); // sort by created
...
async function _writeFile(info, navs, tpl) {
  ...
  console.log(green('\n Compiled ' + info.source + ' into ' + info.htmlFile));
}
